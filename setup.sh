...
if ! [ -d "$TDP_ROLES_PATH" ]; then
  git clone -o StrictHostKeyChecking=no git@github.com:TOSIT-IO/ansible-tdp-roles.git "$TDP_ROLES_PATH"
  # git clone https://github.com/TOSIT-IO/ansible-tdp-roles.git "$TDP_ROLES_PATH"
fi
if ! [ -d "$TDP_ROLES_EXTRA_PATH" ]; then
  git clone -o StrictHostKeyChecking=no git@github.com:TOSIT-IO/tdp-collection-extras.git "$TDP_ROLES_EXTRA_PATH"
  # git clone https://github.com/TOSIT-IO/tdp-collection-extras.git "$TDP_ROLES_EXTRA_PATH"
fi
